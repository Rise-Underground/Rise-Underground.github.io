...
The competition runs 2nd Monday of the month, 00:00 UTC, through the
following Sunday, 23:59:59.999 UTC — one 7-day window per month.
...
# ---------------------------------------------------------------------
# TEST_MODE is off -- the real "2nd Monday of the month" rule below is
# now live. First official window: Sep 14-20, 2026.
# ---------------------------------------------------------------------
TEST_MODE = False
...
def compute_competition_window(now=None):
    ...
    start, end = get_second_monday_window(now.year, now.month)
    if now > end:
        ...
        if next_month > 12:
            next_month = 1
            next_year += 1
        start, end = get_second_monday_window(next_year, next_month)
    return start, end
